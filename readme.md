I'm \*\*VickeKatt\*\* — developer, tinkerer and builder of random things.



\## About me



I enjoy building and experimenting with:



\- Programming

\- Windows, Linux \& system customization

\- Hardware \& electronics

\- ESP32 projects

\- Web development

\- Trying out different technologies



\## Technologies



`Python` · `HTML` · `CSS` · `JavaScript` · `Batch`  

`Bash` · `Git` · `Linux` · `ESP32` · `GitHub` 



\## Projects



\- \*\*\[NexOS](https://github.com/VickeKatt/NexOS)\*\* — A web based "Operating System".



\## Currently



Experimenting with:



\- Linux

\- ESP32 \& electronics

\- Python projects

\- Web development

\- System customization



\## GitHub



\[github.com/VickeKatt](https://github.com/VickeKatt)

